...
Workspace header search is the preferred visible entry point for search inside
the selected workspace. The shell may render the input next to the workspace
title, but query execution, indexing, and provider fan-out remain plugin-owned
behavior.

Workspace tool tabs should be ordered by expected usage frequency and user
orientation value, not alphabetically. The first tabs should help the user
...
Панели становятся разделами окна настроек — единственного места, где живут
настройки. Окно открывается шестерёнкой в строке состояния и вызовом
`api.ui.openSettings(panelId)`, во втором случае сразу на нужном разделе.

Кроме панелей плагинов в нём есть встроенные разделы: «Основное» (язык) и
«Плагины» (переход в Plugin Manager). Поиск ищет и по названиям настроек
внутри разделов, а не только по названиям самих разделов. Открытый раздел
запоминается.

До этого настройки плагина открывались модальным окном внутри Plugin Manager
— страницы про установку плагинов, а не про их настройку, — а язык лежал в
выпадающем меню строки состояния, которое росло с каждым установленным
плагином. Кнопка настроек у карточки плагина в Plugin Manager осталась как
короткий путь.
...
- files/blobs;
- наборы записей плагинов, объявленные в манифесте.

Внутренние метаданные дерева передаются отдельными sync-сущностями; общий запрет
на синхронизацию служебных путей `.verstak` не ослабляется.

### Наборы записей плагина

Всё, что плагин хранит для себя, лежит под `.verstak`, куда сканер vault
намеренно не заходит: это не документы пользователя. Плагин объявляет в
манифесте, какие из его документов — списки записей, и какое поле их
опознаёт:
...
`storage: "settings"` — массив под ключом `key` в настройках плагина;
`storage: "data"` — именованный NDJSON-файл в data-пространстве плагина.

Единица переноса — запись, а не документ. Два устройства, добавившие разные
задачи, дают две записи, а не две версии одного файла, и никого не приходится
спрашивать, какую копию оставить, — тем более что JSON-блоб под `.verstak`
человек не откроет и руками не сольёт. Запись, изменённая на обоих
устройствах, разрешается по `server_sequence`: побеждает последняя. Это то же
правило, по которому работает остальная синхронизация, а не новое, придуманное
здесь.

Операция для плагина, которого на этом устройстве нет, пропускается, а не
отвергается: чужой плагин не должен останавливать чужой pull. Применение
пришедшей записи пишет в хранилище напрямую, поэтому не отправляется обратно, и
объявляется событием `plugin.records.changed`, иначе приехавшая задача
появлялась бы только при следующем открытии вида.

### Что остаётся на устройстве

Не всё, что не синхронизируется, должно синхронизироваться:

- Активность — запись того, что происходило на этой машине. Сливать логи
  событий разных устройств в одну ленту значит выдумывать историю, которой не
  было; предложения для журнала строятся по местной работе.
- Входящие из браузера — очередь приёмника, который слушает на этой машине.
  Разобранная запись превращается в файл или заметку и уезжает уже так.
- Секреты — зашифрованы мастер-паролем; их перенос требует отдельного решения о
  ключах и в этот контракт не входит.
- Настройки интерфейса плагинов — предпочтения конкретной машины.
