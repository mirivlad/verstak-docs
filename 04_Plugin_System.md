...
- search provider;
- activity provider;
- worklog provider — тот, кто умеет предлагать записи журнала. Журнал
  спрашивает всех объявивших, а не Активность по имени;
- importer;
- exporter;
- vault scanner;
- protocol receiver.
...
Bundle может объявить `activate(api)` рядом с `components`. Shell вызывает его
один раз, без монтирования вида, и результат живёт столько же, сколько
приложение. Это единственный способ для плагина отвечать на команды, когда ни
один его вид не открыт: обработчики, зарегистрированные из `mount()`,
существуют ровно пока вид смонтирован. `commands.executeFor` активирует плагин
прежде, чем назвать объявленную команду необработанной.
...
Раздел описывает, что permissions на самом деле дают, а что нет. Без этого
легко принять их за песочницу, которой они не являются.

### Что происходит на самом деле

Frontend-бандл плагина исполняется через `new Function(content)` в том же
JavaScript-контексте, что и оболочка. У него есть доступ к `window`, к DOM
всего приложения и к тем же сетевым возможностям, что и у оболочки. Никакой
изоляции — ни iframe, ни отдельного worker, ни отдельного процесса — между
плагином и ядром нет.

Отсюда следует прямое утверждение: **включённый frontend-плагин обладает
полными правами приложения.** Permissions ограничивают то, что плагин получает
через `VerstakPluginAPI`, а не то, на что он способен.

### Зачем тогда permissions

Они решают три реальные задачи, и стоит называть их честно:

1. **Согласие.** Пользователь видит до включения, что плагин собирается
   делать. `network.remote`, `process.spawn`, `secrets.read` в манифесте — это
   заявление о намерениях, которое можно оценить.
2. **Ошибки, а не злой умысел.** Плагин без `files.write` получит отказ на
   попытку записи. Это ловит опечатки и недоразумения в добросовестном коде.
3. **Аудируемость.** Манифест — предмет ревью. Расширение прав видно в diff.

Чего они не решают: враждебный плагин обходит их, обращаясь к `window`
напрямую. Проверка прав живёт по ту сторону границы, которую он не обязан
пересекать.

### Что из этого следует

- Установка плагина эквивалентна по доверию установке приложения. Так и надо
  об этом говорить в интерфейсе — не «разрешить доступ к файлам», а «запустить
  этот код».
- Официальные плагины поставляются из репозитория, проходящего ревью и CI.
  Это и есть текущая граница доверия — организационная, не техническая.
- Backend-часть плагина (sidecar) — отдельный процесс, и там границы
  настоящие: ядро контролирует, что именно передаётся.

### Что изменит это в будущем

Настоящая изоляция frontend требует исполнения бандла вне общего контекста —
в отдельном worker или iframe с обменом только сообщениями. Это меняет и
контракт API (всё становится асинхронным и сериализуемым), и способ, которым
плагины рисуют интерфейс. Пока это не сделано, раздел выше описывает
действительное положение дел, а не желаемое.
